...
#July 20th, EJR
#Now implementing a small amount of parallelization. More could be done, but this should be enough for now. We are probably not going to need to run this often. 
...
num_processes=$(( max_num_processes / 2 ))
if (( $max_num_processes < 1 )); then
    max_num_processes=1
fi
...
make_binary(){
    ...
    num_files=$(find $1"/C4Test000"*".txt" -maxdepth 1 -type f | wc -l)
    num_new_folders=$(( (num_files / num_files_per_folder) + 1 ))
    if (( $num_new_folders > 1 )); then
        for foldnum in $( seq 1 $num_new_folders ); do #This would be another good place for parallelization, if desired. 
...
for base_name in ${name_array[@]}; do 
 ((i=i%num_processes)); ((i++==0)) && wait #We make sure we don't use more than the allowed number of processes
 make_binary "$base_name" & #The & allows everything to be run in parallel. 
done && wait
